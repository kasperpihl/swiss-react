...
export function addPlugin(name, handler) {
  ...
  if (typeof handler !== 'function') {
    return console.warn(
      'swiss addPlugin: second argument should be the plugin handler'
    );
  }
  ...
  runPlugin('parseRawCss', (handler, i) => {
    ...
    if (typeof res !== 'undefined' && res !== false) {
      if (typeof res !== 'string') {
        return console.warn(
          `swiss plugin parseRawCss[${i}] error: Expected string. Got ${typeof res}`
        );
      }
      rawCss = res;
    }
  ...
  runPlugin('parseKeyValue', (handler, i) => {
    ...
    if (typeof res !== 'undefined' && res !== false) {
      if (!Array.isArray(res)) {
        return console.warn(
          `swiss plugin parseKeyValue[${i}] error: Expected array. Got ${typeof res}`
        );
      }
      if (res.length !== 2 || typeof res[0] !== 'string') {
        return console.warn(
          `swiss plugin parseKeyValue[${i}] error: Expected array with [string, value]`
        );
      }
      key = res[0];
      value = res[1];
  ...
  runPlugin('parseRawInline', (handler, i) => {
    ...
    if (typeof res !== 'undefined') {
      if (typeof res !== 'object') {
        return console.warn(
          `swiss plugin parseRawInline[${i}] error: Expected object. Got ${typeof res}`
        );
      }
      inlineStyles = res;
    }
